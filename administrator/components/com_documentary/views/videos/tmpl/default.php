...
$canOrder	= $user->authorise('core.edit.state', 'com_documentary');
$saveOrder	= $listOrder == 'a.ordering';
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_documentary&task=videos.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_documentary&view=videos'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<table class="table table-striped" id="videoList">
			<thead>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_DOCUMENTARY_VIDEOS_CREATED_BY', 'a.created_by', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                $canCreate	= $user->authorise('core.create',		'com_documentary');
                $canEdit	= $user->authorise('core.edit',			'com_documentary');
                $canCheckin	= $user->authorise('core.manage',		'com_documentary');
                $canChange	= $user->authorise('core.edit.state',	'com_documentary');
				?>
